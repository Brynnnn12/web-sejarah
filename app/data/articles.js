export const articles = [
  {
    id: "proklamasi-kemerdekaan",
    title: "Proklamasi Kemerdekaan",
    desc: "Peristiwa bersejarah pembacaan teks proklamasi kemerdekaan Indonesia",
    date: "17 Agustus 1945",
    image: "/images/articles/proklamasi.jpg",
    category: "Masa Kemerdekaan",
    views: 1024,
    likes: 250,
    content: `
      Proklamasi Kemerdekaan Indonesia dilaksanakan pada hari Jumat tanggal 17 Agustus 1945. 
      Teks proklamasi dibacakan oleh Ir. Soekarno yang didampingi oleh Drs. Mohammad Hatta di 
      sebuah rumah di Jalan Pegangsaan Timur No. 56, Jakarta.

      Sebelum dibacakan, naskah proklamasi diketik oleh Sayuti Melik menggantikan naskah 
      tulisan tangan Soekarno. Tiga perubahan yang dilakukan adalah:
      1. Kata "tempoh" menjadi "tempo"
      2. "Wakil-wakil bangsa Indonesia" menjadi "atas nama bangsa Indonesia"
      3. Tanggal "17-8-05" menjadi "17-8-45"
    `,
  },
  {
    id: "kerajaan-majapahit",
    title: "Kerajaan Majapahit",
    desc: "Kejayaan kerajaan terbesar di Nusantara pada abad ke-13 hingga ke-16",
    date: "1293 - 1500",
    image: "/images/articles/majapahit.jpg",
    category: "Pra-Kemerdekaan",
    views: 875,
    likes: 190,
    content: `
      Majapahit adalah sebuah kerajaan yang berpusat di Jawa Timur, Indonesia, yang pernah 
      berdiri dari sekitar tahun 1293 hingga 1500 M. Kerajaan ini mencapai puncak kejayaannya 
      pada masa pemerintahan Hayam Wuruk.
    `,
  },
  {
    id: "sumpah-pemuda",
    title: "Sumpah Pemuda",
    desc: "Momentum persatuan pemuda Indonesia yang mengikrarkan satu tanah air, bangsa dan bahasa",
    date: "28 Oktober 1928",
    image: "/images/articles/sumpah-pemuda.jpg",
    category: "Pra-Kemerdekaan",
    views: 980,
    likes: 230,
    content: `
      Sumpah Pemuda adalah satu tonggak utama dalam sejarah pergerakan kemerdekaan Indonesia. 
      Ikrar ini diucapkan pada tanggal 28 Oktober 1928 dan dianggap sebagai kristalisasi 
      semangat untuk menegaskan cita-cita berdirinya negara Indonesia.
    `,
  },
  {
    id: "reformasi-1998",
    title: "Reformasi 1998",
    desc: "Gerakan reformasi yang mengubah sistem pemerintahan Indonesia",
    date: "Mei 1998",
    image: "/images/articles/reformasi.jpg",
    category: "Era Modern",
    views: 1120,
    likes: 300,
    content: `
      Reformasi 1998 adalah masa perubahan besar dalam sejarah Indonesia yang ditandai 
      dengan berakhirnya masa Orde Baru dan dimulainya era Reformasi.
    `,
  },
  {
    id: "sultan-agung-melawan-voc",
    title: "Sultan Agung Melawan VOC",
    desc: "Perjuangan Sultan Agung dalam melawan kekuasaan VOC di Nusantara",
    date: "1628 - 1629",
    image: "/images/articles/sultan-agung.jpg",
    category: "Pra-Kemerdekaan",
    views: 720,
    likes: 170,
    content: `
      Sultan Agung dari Mataram adalah salah satu pemimpin terbesar dalam sejarah Nusantara. 
      Ia berupaya mengusir VOC yang mulai menancapkan kekuasaannya di Batavia. 

      Pada tahun 1628, Sultan Agung mengirim pasukan besar untuk menyerang Batavia, 
      tetapi serangan ini gagal akibat kurangnya persediaan logistik. Tak menyerah, 
      pada tahun 1629 ia kembali menyerang dengan strategi yang lebih matang, termasuk 
      pemutusan jalur suplai air ke kota. Namun, kekuatan militer VOC yang unggul 
      dan wabah penyakit akhirnya menggagalkan upaya ini.

      Meski gagal merebut Batavia, perlawanan Sultan Agung menjadi simbol perlawanan terhadap 
      kolonialisme dan memberikan inspirasi bagi perjuangan kemerdekaan Indonesia di masa mendatang.
    `,
  },
  {
    id: "pertempuran-ambarawa",
    title: "Pertempuran Ambarawa",
    desc: "Pertempuran heroik pasukan Indonesia melawan tentara Sekutu di Ambarawa",
    date: "20 November 1945",
    image: "/images/articles/ambarawa.jpg",
    category: "Masa Kemerdekaan",
    views: 860,
    likes: 210,
    content: `
      Pertempuran Ambarawa adalah salah satu pertempuran penting dalam sejarah kemerdekaan Indonesia. 
      Terjadi pada November hingga Desember 1945, pertempuran ini melibatkan pasukan Indonesia yang 
      dipimpin oleh Jenderal Soedirman dalam melawan tentara Sekutu yang ingin menguasai wilayah Ambarawa.
      
      Melalui taktik gerilya dan strategi serangan serentak, pasukan Indonesia berhasil mengepung 
      dan memukul mundur pasukan Sekutu. Kemenangan ini menjadi salah satu bukti kuatnya tekad 
      rakyat Indonesia dalam mempertahankan kemerdekaan.
    `,
  },
  {
    id: "pendudukan-jepang",
    title: "Pendudukan Jepang di Indonesia",
    desc: "Periode penjajahan Jepang di Indonesia dari tahun 1942 hingga 1945",
    date: "1942 - 1945",
    image: "/images/articles/pendudukan-jepang.jpg",
    category: "Pra-Kemerdekaan",
    views: 930,
    likes: 220,
    content: `
      Pendudukan Jepang di Indonesia berlangsung dari tahun 1942 hingga 1945 setelah Jepang 
      berhasil mengalahkan Belanda dalam Perang Dunia II. Jepang awalnya datang dengan janji 
      membebaskan Indonesia dari kolonialisme Belanda, namun segera menerapkan kebijakan 
      yang lebih keras, termasuk kerja paksa (romusha) dan penindasan terhadap rakyat.

      Meskipun masa pendudukan ini penuh penderitaan, hal ini juga memicu kesadaran nasionalisme 
      yang lebih kuat di kalangan rakyat Indonesia, yang akhirnya berkontribusi pada proklamasi 
      kemerdekaan pada 17 Agustus 1945.
    `,
  },
];
